...
let
  ...
  inherit (pkgs.callPackage ../modules/lib/tf_state.nix { }) resourcesByRole nodeIP;
  inherit (pkgs.callPackage ../modules/lib/const.nix { }) virtualIP;
  ...
  self =
    if get_resource == [ ] then { values.ipv4_address = "dummy"; } else builtins.head get_resource;
in
{
  _module.args = {
    inherit (pkgs.callPackage ../modules/lib/consts.nix { }) workspace constByKey;
    inherit (pkgs.callPackage ../modules/lib/tf_state.nix { })
      resources
      resourcesByRole
      resourcesByRoles
      machineType
