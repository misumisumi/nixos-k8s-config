...
let
  ...
  get_resource = builtins.filter (r: r.values.name == name) (
    resourcesByRole (lib.trace name name) "ipxe-server"
  );
  self =
...
in
{
  _module.args = {
    ...
    inherit (pkgs.callPackage ../../../utils/resources.nix { })
      resources
      resourcesByRole
      resourcesByRoles
      machineType
      ;
    nodeIP = lib.trace (nodeIP self) (nodeIP self);
  };
}
