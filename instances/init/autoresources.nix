...
let
  ...
  inherit (pkgs.callPackage ../../utils/resources.nix { }) resources nodeIP;
  ...
  get_resource = builtins.filter (r: r.values.name == name) (resources "netboot");
  self = if get_resource == [ ] then { values.ip_address = "dummy"; } else builtins.head get_resource;
in
{
  _module.args = {
    inherit (pkgs.callPackage ../../utils/consts.nix { }) workspace;
