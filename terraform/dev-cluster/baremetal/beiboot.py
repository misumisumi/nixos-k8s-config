...
class AuthorizeResponder(ferny.AskpassHandler):
    commands = (
        "ferny.askpass",
        "cockpit.report-exists",
        "cockpit.fail-no-cockpit",
    ...
    async def do_askpass(self, messages: str, prompt: str, hint: str) -> Optional[str]:
        logger.debug(
            "AuthorizeResponder: prompt %r, messages %r, hint %r",
            prompt,
            messages,
            hint,
        )
        ...
        if self.have_basic_password and "password:" in prompt.lower():
            ...
            if self.basic_password is not None:
                logger.debug(
                    "AuthorizeResponder: sending Basic auth password for prompt %r",
                    prompt,
                )
        ...
        response = await self.router.request_authorization(
            challenge,
            timeout=None,
            messages=messages,
            prompt=prompt,
    ...
    async def do_custom_command(
        self, command: str, args: tuple, fds: list[int], stderr: str
    ) -> None:
        ...
        if command == "cockpit.report-exists":
            ...
            self.router.routing_rules.insert(
                0, ChannelRoutingRule(self.router, [PackagesChannel])
            )
        ...
        if command == "cockpit.check-os-release":
            ...
            for osinfo in supported_oses:
                ...
                if all(remote_os.get(k) == v for k, v in osinfo.items()):
                    logger.debug(
                        "cockpit.check-os-release: remote matches supported OS %r",
                        osinfo,
                    )
                    return
            ...
            try:
                ...
                logger.warning(
                    "failed to read local /etc/os-release, skipping OS compatibility check: %s",
                    e,
                )
                return

            if remote_os.get("ID") == this_os.get("ID") and remote_os.get(
                "VERSION_ID"
            ) == this_os.get("VERSION_ID"):
                logger.debug(
                    "cockpit.check-os-release: remote OS matches local OS %r", this_os
                )
                return
...
def python_interpreter(comment: str) -> tuple[Sequence[str], Sequence[str]]:
    print("python3", "-ic", f"# {comment}")
    return ("python3", "-ic", f"# {comment}"), ()


def via_ssh(
    cmd: Sequence[str], dest: str, ssh_askpass: Path, *ssh_opts: str
    ...
    return ("ssh", *ssh_opts, *destination, shlex.join(cmd)), (
...
def flatpak_spawn(
    cmd: Sequence[str], env: Sequence[str]
) -> tuple[Sequence[str], Sequence[str]]:
    return ("flatpak-spawn", "--host", *(f"--env={kv}" for kv in env), *cmd), ()


class SshPeer(Peer):
    mode: (
        'Literal["always"] | Literal["never"] | Literal["supported"] | Literal["auto"]'
    )
    ...
    async def boot(self, cmd: Sequence[str], env: Sequence[str]) -> None:
        ...
        agent = ferny.InteractionAgent(
            [AuthorizeResponder(self.router, self.basic_password), beiboot_helper]
        )
    ...
    def do_authorize(self, message: JsonObject) -> None:
        logger.debug(
            "SshPeer.do_authorize: %r; have password %s",
            message,
            self.basic_password is not None,
        )
        if get_str(message, "challenge").startswith("plain1:"):
            ...
            if self.basic_password is not None:
                ...
                self.write_control(
                    command="authorize", cookie=cookie, response=self.basic_password
                )
        ...
        self.write_control(
            command="authorize", cookie=cookie, problem="authentication-unavailable"
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="cockpit-bridge is run automatically inside of a Cockpit session."
    )
    parser.add_argument(
        "--remote-bridge",
        choices=["auto", "never", "supported", "always"],
        default="auto",
    ...
    parser.add_argument(
        "destination", help="Name of the remote host to connect to, or 'localhost'"
    )
