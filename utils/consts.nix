...
rec {
  ...
  nodeIPsByRole =
    role: lib.filterAttrs (x: y: lib.hasPrefix role x) (constByKey "instanceIPs").${workspace};
  nodeIPsByRoles =
    roles:
      ...
      lib.flatten (
        lib.forEach roles (
          ...
          lib.mapAttrsToList (x: y: {
            name = x;
            value = y;
          }) (lib.filterAttrs (x: y: lib.hasPrefix role x) (constByKey "instanceIPs").${workspace})
        )
      )
    );
  nodeIPByWS = node: ws: (constByKey "instanceIPs").${ws}.${node};
  nodeIPsByRoleAndWS =
    role: ws: lib.filterAttrs (x: y: lib.hasPrefix role x) (constByKey "instanceIPs").${ws};
}
